...
let imagesPath = import.meta.dirname + "/../static/images";
let imageList = walkSync(imagesPath + '/', null);

imageList.forEach(i => {
    return; // disable image resize for now
    ...
    let smallPath = path.dirname(i) + "/" + path.basename(i).split('.')[0] + "_small" + path.extname(i);
    ...
    smallPath = path.dirname(i) + "/" + path.basename(i).split('.')[0] + "_xsmall" + path.extname(i);
    if (fs.existsSync(smallPath))
    {
        // already there - do nothing
        return;
